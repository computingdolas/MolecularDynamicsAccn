...
int main(int argc, const char * argv[]) {
    ...
    // Algorithm to follow
    {
        ...
        updateListsCellPar<<<1,numcells>>>(cell_list.devicePtr,particle_list.devicePtr,position.devicePtr,const_args.devicePtr,num_cells.devicePtr,numparticles);
        //calculate Initial forces
        calcForcesCellPar<<<num_blocks,threads_per_blocks>>>(forcenew.devicePtr,position.devicePtr,sigma,epsilon,r_cut,const_args.devicePtr,\
                                                             num_cells.devicePtr,cell_list.devicePtr,particle_list.devicePtr);
        for(real_d t =0.0 ; t < time_end ; t+= timestep_length ) {
            ...
            setToZero<<<num_blocks,threads_per_blocks>>>(forcenew.devicePtr,numparticles);
            calcForcesCellPar<<<num_blocks,threads_per_blocks>>>(forcenew.devicePtr,position.devicePtr,sigma,epsilon,r_cut,const_args.devicePtr,num_cells.devicePtr,cell_list.devicePtr,particle_list.devicePtr);
